...
from commands.RPG.utils.database import equip_item, get_active_hero, list_active_inventory
...
from commands.RPG.utils.presentation import RPG_PRIMARY_COLOR, build_item_instance, get_rarity_label
...
class Equip(commands.Cog):
    ...
    def _build_selector_embed(display_name: str, slot_name: str, inventory_rows: list[dict], equipped_item_id: int | None):
        ...
        embed.description = f"Selecione abaixo qual {slot_name.lower()} ficará ativa no herói de {display_name}."
        ...
        if equipped_item_id is not None:
            ...
            if equipped_row is not None:
                ...
                if equipped_item is not None:
                    equipped_name = f"{equipped_item.name} (nível {equipped_row['level']})"
        embed.add_field(name="Atualmente equipado", value=equipped_name, inline=False)
        embed.add_field(name="Itens disponíveis", value=str(len(inventory_rows)), inline=True)
        embed.add_field(name="Dica", value="Priorize itens com nível e bônus melhores para aumentar o poder do herói.", inline=True)
        return embed
                ...
                for row in rows[:25]:
                    ...
                    options.append(
                        SelectOption(
                            ...
                            emoji="⚔️" if item_type_id == 1 else "🛡️",
                        )
                    )
