...
from commands.RPG.utils.database import ensure_profile, get_active_hero, set_active_hero_race
...
class Race(commands.Cog):
    ...
    async def escolher_raca(self, ctx, *, raca: Optional[str] = None):
        ...
        if raca is None:
            options = ", ".join(RACES)
            return await inte.response.send_message(f"Informe uma raça valida. Opcoes: {options}")
