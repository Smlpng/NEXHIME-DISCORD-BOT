...
class Weekly(commands.Cog):
    ...
    async def weekly(self, ctx: commands.Context):
        ...
        if hero is None:
            return await inte.response.send_message("Você precisa ter um herói ativo para resgatar a recompensa semanal.")
        ...
        if available_at > now_ts:
            await inte.response.send_message(
                f"Você já resgatou seu semanal. Tente novamente <t:{int(available_at)}:R>.",
                ephemeral=True,
            )
            return
        ...
        if not ok:
            return await inte.response.send_message("Não consegui aplicar a recompensa agora. Tente novamente.")
        ...
        embed.add_field(name="Próximo resgate", value="Daqui a 7 dias", inline=True)
