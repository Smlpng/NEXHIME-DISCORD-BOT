...
def get_item_type_emoji(item_type_id: int) -> str:
    return ITEM_TYPE_EMOJIS.get(item_type_id, "🎒")
...
def format_item_summary(item_row: dict, equipped_item_id: int | None = None) -> str:
    ...
    if item is None:
        return "Item inválido ou removido do catálogo."
    ...
    return (
        ...
        f"Nível {item_row['level']} | {get_rarity_label(getattr(item, 'rarity', None))}\n"
        f"Bônus: {item.boosts}\n"
...
def build_inventory_embed(display_name: str, inventory_rows: list[dict], hero_data: dict | None = None) -> discord.Embed:
    embed = discord.Embed(title=f"Inventário de {display_name}", color=RPG_PRIMARY_COLOR)

    if not inventory_rows:
        embed.description = "Seu inventário está vazio no momento. Passe na loja, lute ou melhore itens na forja para começar a montar seu arsenal."
        return embed
