...
class Bank(commands.Cog):
    ...
    async def depositar(self, ctx, amount: int):
        ...
        if get_active_hero(inte.user.id) is None:
            ...
            return await inte.response.send_message("Seu perfil economico foi criado, mas você ainda não tem um herói para depositar nex da carteira.")
        ...
        if not deposit_nex_to_bank(inte.user.id, amount):
            ...
            return await inte.response.send_message("Você não tem nex suficiente na carteira para esse deposito.")
    ...
    async def sacar(self, ctx, amount: int):
        ...
        if get_active_hero(inte.user.id) is None:
            ...
            return await inte.response.send_message("Seu perfil economico foi criado, mas você ainda não tem um herói para receber o saque de nex na carteira.")
        ...
        if not withdraw_nex_from_bank(inte.user.id, amount):
            ...
            return await inte.response.send_message("Você não tem saldo suficiente em nex no banco para esse saque.")
    ...
    async def transferir(self, ctx, member: discord.Member, amount: int):
        ...
        if member.bot:
            ...
            return await inte.response.send_message("Você não pode transferir nex para bots.")

        if member.id == inte.user.id:

            return await inte.response.send_message("Você não pode transferir nex para si mesmo.")
        ...
        if target_hero is None:
            ...
            return await inte.response.send_message(
                ...
                f"{member.display_name} ainda não tem um herói ativo para receber nex."
        ...
        if not transfer_wallet_nex(inte.user.id, member.id, amount):
            ...
            return await inte.response.send_message(
                ...
                f"Você não tem nex suficiente na carteira para essa transferencia. Carteira atual: {format_nex(wallet)} nex."
