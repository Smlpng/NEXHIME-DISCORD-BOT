...
class Mercado(commands.Cog):
    ...
    async def mercado(self, ctx, acao: str | None = None, *args):
        ...
        """Mercado assíncrono de recursos do RPG."""
        ...
        if get_active_hero(inte.user.id) is None:
            ...
            return await inte.response.send_message("Você precisa ter um herói ativo para usar o mercado.")
        ...
        if action in {"listar", "list"}:
            ...
            if not offers:
                ...
                embed.description = "Não ha ofertas abertas no momento."
        ...
        if action == "criar":
            ...
            try:
                ...
                return await inte.response.send_message("As quantidades precisam ser números inteiros.")
            ...
            if int(hero.get(give_info[0], 0)) < give_amount:
                ...
                return await inte.response.send_message("Você não tem recursos suficientes para abrir essa oferta.")



            if not update_active_hero_resources(inte.user.id, **{give_info[0]: -give_amount}):

                return await inte.response.send_message("Não foi possível reservar os recursos dessa oferta.")
        ...
        if action == "cancelar":
            ...
            if offer is None:
                ...
                return await inte.response.send_message("Oferta não encontrada ou já encerrada.")
        ...
        if action == "aceitar":
            ...
            if offer is None:
                ...
                return await inte.response.send_message("Oferta não encontrada ou indisponivel.")

            if offer["seller_id"] == inte.user.id:

                return await inte.response.send_message("Você não pode aceitar a propria oferta.")
            ...
            if buyer is None or seller is None:
                ...
                return await inte.response.send_message("Um dos perfis envolvidos não possui herói ativo.")

            if int(buyer.get(offer["receive"], 0)) < offer["receive_amount"]:

                return await inte.response.send_message("Você não tem recursos suficientes para aceitar essa oferta.")



            if not update_active_hero_resources(inte.user.id, **{offer["receive"]: -offer["receive_amount"], offer["give"]: offer["give_amount"]}):

                return await inte.response.send_message("Não foi possível concluir a compra.")
