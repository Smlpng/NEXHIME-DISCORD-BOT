...
class Trade(commands.Cog):
    ...
    async def trade(self, ctx, give: str, give_amount: int, receive: str, receive_amount: int):
        ...
        if get_active_hero(inte.user.id) is None:
            ...
            return await inte.response.send_message("Seu perfil economico foi criado, mas você precisa criar um herói antes de trocar recursos do jogo.")
        ...
        if give == receive:
            ...
            return await inte.response.send_message("A quantidade de recursos não pode ser negativa.", ephemeral=True)

        elif give_amount == 0 and receive_amount == 0:

            return await inte.response.send_message("As duas quantidades não podem ser 0 ao mesmo tempo.", ephemeral=True)
        ...
        if data[resource_column] < give_amount:
            ...
            return await inte.response.send_message("Você não tem recursos suficientes para essa troca.", ephemeral=True)
    ...
    async def make_trade(self, interaction, inte, give : int, give_amount : int, receive : int, receive_amount : int):
        ...
        if interaction.user.id == inte.user.id:
            ...
            return await interaction.response.send_message("Você não pode aceitar a propria oferta de troca.", ephemeral=True)
        ...
        if data[receive_column] < receive_amount:
            ...
            return await interaction.response.send_message("Você não tem recursos suficientes para aceitar essa troca.", ephemeral=True)
        ...
        if data[resource_column] < give_amount:
            ...
            return await interaction.response.send_message("A oferta original não e mais valida.", ephemeral=True)
