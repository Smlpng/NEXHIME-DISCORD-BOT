...
from commands.EconomiaRPG.utils.database import clear_active_hero_class, ensure_profile, has_selected_class, update_active_hero_resources
...
class ResetClasse(commands.Cog):
    ...
    async def reset_classe(self, ctx: commands.Context):
        ...
        if not has_selected_class(inte.user.id):
            return await inte.response.send_message("VocÃª ainda nÃ£o escolheu uma classe.")
        ...
        if RESPEC_COST_NEX > 0:
            ...
            if not ok:
                return await inte.response.send_message(
                    f"VocÃª precisa de **{RESPEC_COST_NEX} nex** para resetar sua classe.",
                    ephemeral=True,
                )

        clear_active_hero_class(inte.user.id)
        embed = discord.Embed(title="ðŸ”„ Classe resetada", color=RPG_PRIMARY_COLOR)
        embed.description = (
