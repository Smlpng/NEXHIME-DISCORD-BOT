...
class Tribe(commands.Cog):
    ...
    async def _sync_tribe_role(
        ...
        return f"ParabÃ©ns! VocÃª entrou para **{desired_role.name}**, agora vÃ¡, explore, conquiste e honre sua tribo!"
    ...
    async def escolher_tribo(self, ctx, *, tribo: Optional[str] = None):
        """Define a tribo do herÃ³i atual."""
        ...
        if tribo is None:
            ...
            await inte.response.send_message("Escolha a tribo do seu herÃ³i:", view=view, ephemeral=True)
            await view.wait()
    ...
    async def trocar_tribo(self, ctx, *, nova_tribo: Optional[str] = None):
        """Troca a tribo do herÃ³i atual dentro do sistema novo."""
        inte = CommandContextAdapter(ctx)
        ...
        if nova_tribo is None:
            ...
            await inte.response.send_message("Escolha a nova tribo do seu herÃ³i:", view=view, ephemeral=True)
            await view.wait()
            if view.selected_value is None:
                return await inte.followup.send("Tempo esgotado. Tente novamente quando quiser.", ephemeral=True)
