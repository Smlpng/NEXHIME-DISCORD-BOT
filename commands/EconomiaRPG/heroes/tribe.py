...
from commands.EconomiaRPG.utils.database import ensure_profile, get_active_hero, set_active_hero_tribe
...
class Tribe(commands.Cog):
    ...
    async def _sync_tribe_role(
        ...
        if member is None:
            ...
            return "A tribo foi salva, mas não consegui localizar seu membro no servidor para ajustar o cargo."
        ...
        if desired_role is None:
            ...
            return f"A tribo foi salva, mas o cargo de **{selected_tribe}** não está configurado no servidor."
        ...
        if me is None or me.top_role <= desired_role:
            ...
            return f"A tribo foi salva, mas não consigo entregar o cargo **{desired_role.name}** por causa da hierarquia."
        ...
        try:
            ...
            return f"A tribo foi salva, mas faltou permissão para ajustar o cargo **{desired_role.name}**."
    ...
    async def escolher_tribo(self, ctx, *, tribo: Optional[str] = None):
        ...
        if hero is None:
            ...
            return await inte.response.send_message("Seu perfil ainda não está pronto para escolher uma tribo.")



        if hero.get("tribe"):

            return await inte.response.send_message(f"Seu herói já pertence a {hero['tribe']}.", ephemeral=True)
    ...
    async def trocar_tribo(self, ctx, *, nova_tribo: Optional[str] = None):
        ...
        if hero is None:
            ...
            return await inte.response.send_message("Seu perfil ainda não está pronto para trocar de tribo.")



        if not hero.get("tribe"):

            return await inte.response.send_message("Seu herói ainda não escolheu uma tribo. Use escolher_tribo primeiro.", ephemeral=True)
        ...
        if selected_tribe == hero.get("tribe"):
            ...
            return await inte.response.send_message("Seu herói já está nessa tribo.", ephemeral=True)
