...
class RaceSelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="Selecione sua raça", min_values=1, max_values=1, options=options)
...
class Race(commands.Cog):
    ...
    async def escolher_raca(self, ctx, *, raca: Optional[str] = None):
        ...
        if hero is None:
            return await inte.response.send_message("Seu perfil ainda não está pronto para escolher uma raça.")

        if hero.get("race"):
            return await inte.response.send_message(f"Seu herói já tem uma raça definida: {hero['race']}.", ephemeral=True)
        ...
        if raca is None:
            options = ", ".join(RACES)
            return await inte.response.send_message(f"Informe uma raça válida. Opções: {options}")

        selected_race = next((item for item in RACES if item.lower() == raca.lower()), None)
        if selected_race is None:
            return await inte.response.send_message("Raça inválida. Use uma das opções mostradas pelo comando.", ephemeral=True)

        set_active_hero_race(inte.user.id, selected_race)

        embed = discord.Embed(title="Raça definida", color=discord.Color.blue())
        embed.add_field(name="Herói", value=inte.user.display_name, inline=True)
