...
from commands.EconomiaRPG.utils.database import get_active_hero, list_active_clean_inventory
from commands.EconomiaRPG.items.weapons import weapon_dict
from commands.EconomiaRPG.items.armors import armor_dict
from commands.EconomiaRPG.utils.equipment_upgrade import equipment_upgrade_cost, make_upgrade
...
class Forge(commands.Cog):
    ...
    def create_upgrade_embed(self, inte, cost, user_data, item, name):
        ...
        embed.add_field(name="Nex ðŸ’°", value=cost[0], inline=True)
        embed.add_field(name="Madeira ðŸŒ²", value=cost[1], inline=True)
        embed.add_field(name="Ferro â›ï¸", value=cost[2], inline=True)
        embed.add_field(name="Runas ðŸ§¿", value=cost[3], inline=True)
        embed.add_field(name="Inventario", value="", inline=False)
        embed.add_field(name="Nex ðŸ’°", value=user_data["nex"], inline=True)
        embed.add_field(name="Madeira ðŸŒ²", value=user_data["wood"], inline=True)
        embed.add_field(name="Ferro â›ï¸", value=user_data["iron"], inline=True)
        embed.add_field(name="Runas ðŸ§¿", value=user_data["runes"], inline=True)
