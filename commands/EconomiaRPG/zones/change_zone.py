...
class ChangeZone(commands.Cog):
    ...
    async def change_zone(self, ctx):
                    ...
                    if hero.level >= 20:
                        ...
                        if hero.level >= 25:
                            ...
                            if hero.level >= 35:
                                ...
                                options.append(SelectOption(value=0, label="Uma nova zona sera liberada no nível 35", emoji='ðŸ”’'))

                        else:

                            options.append(SelectOption(value=0, label="Uma nova zona sera liberada no nível 25", emoji='ðŸ”’'))

                    else:

                        options.append(SelectOption(value=0, label="Uma nova zona sera liberada no nível 20", emoji='ðŸ”’'))

                else:

                    options.append(SelectOption(value=0, label="Uma nova zona sera liberada no nível 10", emoji='ðŸ”’'))
        ...
        await inte.response.send_message('Selecione a zona para onde desejá ir:', view=view, embed=get_zone_embed(data["zone_id"]))
