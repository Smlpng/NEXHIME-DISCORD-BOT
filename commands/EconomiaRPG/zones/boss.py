...
from commands.EconomiaRPG.zones.zones.encounters import get_boos_from_zone
    ...
    async def boss(self, ctx: commands.Context):
        ...
        if available_at > now_ts:
            return await inte.response.send_message(
                f"â³ VocÃª jÃ¡ enfrentou um boss recentemente. Tente novamente <t:{available_at}:R>.",
                ephemeral=True,
            )
        ...
        try:
            ...
            msg = f"VocÃª encontrou **{getattr(enemy, 'name', 'um boss')}** (nÃ­vel {getattr(enemy, 'level', '?')}) e voltou em seguranÃ§a."

        zone_name = resolve_zone_name(int(zone_id))
        embed = discord.Embed(title="ðŸ‘‘ Boss", color=RPG_PRIMARY_COLOR)
        embed.add_field(name="Local", value=zone_name, inline=False)
        embed.add_field(name="Boss", value=f"**{getattr(enemy, 'name', 'Boss')}** (nÃ­vel {getattr(enemy, 'level', '?')})", inline=False)
