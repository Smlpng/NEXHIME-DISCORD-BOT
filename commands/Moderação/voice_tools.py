...
class VoiceTools(commands.Cog):
    ...
    async def voicekick(self, ctx: commands.Context, membro: discord.Member, *, motivo: str | None = None):
        if not self._hierarchy_ok(ctx, membro):
            return await ctx.reply("Não e possível moderar este membro na call.")
        if membro.voice is None or membro.voice.channel is None:
            return await ctx.reply("Esse membro não está em um canal de voz.")
        try:
            await membro.move_to(None, reason=motivo or f"Voice kick por {ctx.author}")
        except discord.Forbidden:
            return await ctx.reply("Faltam permissões para remover este membro da call.")
        except discord.HTTPException as exc:
            return await ctx.reply(f"Não foi possível remover da call: {exc}")
    ...
    async def voicemute(self, ctx: commands.Context, membro: discord.Member, *, motivo: str | None = None):
        ...
        try:
            ...
            return await ctx.reply("Faltam permissões para silenciar este membro.")
        except discord.HTTPException as exc:
            return await ctx.reply(f"Não foi possível silenciar na call: {exc}")
    ...
    async def voiceunmute(self, ctx: commands.Context, membro: discord.Member, *, motivo: str | None = None):
        ...
        try:
            ...
            return await ctx.reply("Faltam permissões para remover o silencio deste membro.")
        except discord.HTTPException as exc:
            return await ctx.reply(f"Não foi possível remover o silencio na call: {exc}")
        await ctx.reply(f"{membro.mention} não está mais silenciado no canal de voz.")
